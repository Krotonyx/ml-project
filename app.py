...
# --- CUSTOM CSS (Full Page Glass Overrides) ---
st.markdown("""
    <style>
    /* 1. Background Image on the App */
    .stApp {
    ...
    /* 2. Style the main content area to BE the glass card */
    .block-container {
    ...
    h1 { font-size: 2.3rem !important; font-weight: 700 !important; color: white !important;}
    ...
    /* Make prediction results look good on glass */
...
CSV_FILE = 'Worldwide Travel Cities Dataset (Ratings and Climate).csv'
...
if df_data is None:
    st.error(f"Error: Could not find '{CSV_FILE}'. Please ensure the file is in the same folder as this script.")
else:
    ...
    if st.button("Generate Recommendations"):
        if not trip_types:
            ...
            pred_budget_idx = model_gbc.predict([avg_prefs + region_values])[0]
            budget_labels = {0: 'Budget', 1: 'Mid-range', 2: 'Luxury'}
            
            st.success(f"Our Model suggests a **{budget_labels[pred_budget_idx]}** budget level.")
